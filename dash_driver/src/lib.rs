...
const DEVICE_NAME_UTF16: [u16; 20] = [
    ...
    0x005C,
    0x0044, 0x0041, 0x0053, 0x0048,
    0x0044, 0x0072, 0x0069, 0x0076, 0x0065, 0x0072,
    0x0000,
];
...
// ---------- Dynamic Blacklist ----------
static mut BLACKLIST: [[u8; MAX_NAME_LENGTH]; MAX_BLACKLIST_ENTRIES] = [[0u8; MAX_NAME_LENGTH]; MAX_BLACKLIST_ENTRIES];
...
unsafe fn init_unicode_string(dest: *mut UNICODE_STRING, src: *const u16) {
    let mut len: u16 = 0;
    while *src.add(len as usize / 2) != 0 {
        len += 2;
    }
    (*dest).Length = len;
    (*dest).MaximumLength = len + 2;
